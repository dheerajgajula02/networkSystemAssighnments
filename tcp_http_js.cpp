...
int main()  {
    ...
    if (pid == 0) {
        ...
        string filepath;
            ...
            if (f) {
                ...
                if (has_suffix( filepath,".js")) {
                    string f_body = 
                    "<!DOCTYPE html> <html> <head><title>JS Runner</title></head> <body> <script>";
                    f_body += "const jsCode = ` ";
                    f_body += body;
                    f_body += "`;" ;
                    f_body += " eval(jsCode) </script> </body> </html>";
                    body = f_body;
                }
        ...
        // send(newSocket, http_response, strlen(http_response), 0);

        // this is fine  just upgrading it to strings :)
